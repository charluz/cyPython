...
def cbfnButtonReset():
    ...
    # btnRaw.config(text='Open RAW', command=open_raw_image, bg='LightGreen')
...
def convert_packed10(raw_array, nbits):
	total_bits = gRawWidth * gRawHeiht * nbits
...
def cbfnButtonLoadRaw(rawArray, is_packed):
    try:
        packed_convert = {
            10:convert_packed10,
            12:convert_packed12,
        ...
        if is_packed:
            fn_convert = packed_convert.get(gRawBits, None)
            fn_convert(rawArray, gRawBits)
        else:
        	splitBayerRawU16(rawArray, gRawWidth, gRawHeight, gRawBits, gRawBayerType)
    except:
        cbfnButtonReset()
        return
...
###########################################################
def open_raw_image(rawFileName, is_packed):
    global gimgRaw, gRawImgFile, gRawBaseName

    bits = gRawBits
    try:
        if bits > 8 and not is_packed:
